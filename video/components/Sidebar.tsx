'use client'
//usePathname works only with client components so it is important to use 'use Client' otherwise it would be treated as server component 
    ...
    <section className="sticky bg-dark-1 left-0 top-0 flex h-screen w-fit flex-col justify-between p-6  pt-28 text-white max-sm:hidden lg:w-[240px]">
     ...
     {sidebarLinks.map((link)=>{
         ...
         return(
           ...
           href={link.route} 
           key={link.label} 
           className={cn('flex gap-4 items-center p-3 rounded-lg justify-start ' , {'bg-light-1':isActive})}
           >
            <Image
            src={link.imgUrl} 
            alt={link.label} 
